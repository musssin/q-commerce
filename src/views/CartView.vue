...
<template>
  <v-container>
    ...
    <v-row>
      <v-col cols="8">
        <CartItem :product="p" v-for="p of cartStore.cart.products"/>
      </v-col>
      <v-col cols="4">
        <v-card
    class="mx-auto"
  ...
  >
    <v-card-item>
      <div>
        ...
        <div class="text-h5 mb-1 " style="color: ;">{{ total }} KZT</div>
      </div>
    </v-card-item>

    <v-card-actions>
      <v-btn variant="outlined" @click="cartStore.showPaymentDialog = true">
        Оформить покупку
      </v-btn>
      <v-btn variant="flat" @click="cartStore.removeAllFromCart()">
        Очистить
      </v-btn>
    </v-card-actions>
  </v-card>
