<template>
  <v-dialog v-model="cart.showPaymentDialog" width="500" @keyup.enter="submit">
    <v-card class="ma-3">
      ...
      <v-text-field class="ml-6 mr-6" type="number" v-model="number" label=" Номер карты"
        :rules="[rules.required]"></v-text-field>
      <v-text-field class="ml-6 mr-6" type="text" v-model="username" label="Имя держателя"
        :rules="[rules.required]"></v-text-field>
      <v-row>
        <v-col cols="6">
          <v-text-field class="ml-6 mr-6" type="text" v-model="date" label="Срок действия" :rules="[rules.required]" />
        </v-col>
        <v-col cols="6">
          <v-text-field class="ml-2 mr-6" type="text" v-model="cvv" label="CVV " :rules="[rules.required]" />
        </v-col>
      </v-row>
      <v-divider :color="'black'" class="ma-2" />
      <v-text-field class="ml-6 mr-6" type="text" v-model="address" label="Адрес доставки"
        :rules="[rules.required]"></v-text-field>
